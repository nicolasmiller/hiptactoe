var Hiptactoe = (function() {
    ...
    exports.node_with_move = function(old_node, move, state) {
        var new_node = [],
        i,
        x,
        y;
        ...
        for(x = 0; x < board_size; x++) {
            for(y = 0; y < board_size; y++) {
                new_node[x][y] = old_node[x][y];
            }
        }
        new_node[move[0]][move[1]] = state;
        return new_node;
    }
    ...
    exports.minimax = function(node, maximizing_player, record_move, depth) {
        ...
        if(maximizing_player) {
            ...
            for(i = 0; i < moves.length; i++) {
                console.log(i);
                child_node = exports.node_with_move(node, moves[i], BOARD_STATE.O);
                console.log(moves[i]);
                console.log(child_node);
                child_value = exports.minimax(child_node, false, false, depth + 1);
            ...
            moves = exports.get_possible_moves(node);
            for(i = 0; i < moves.length; i++) {
                child_node = exports.node_with_move(node, moves[i], BOARD_STATE.X);
                child_value = exports.minimax(child_node, true, false, depth + 1);
                if(child_value > best_value) {
                    best_value = child_value;
                }
            }
    ...
    function init_processing(p) {
        ...
        p.mouseClicked = function() {
            if(game_state != GAME_STATE.PLAYING) {
                ...
                var x = Math.floor(p.mouseX / (cell_width + divider_width));
                var y = Math.floor(p.mouseY / (cell_width + divider_width));
    ...
    function make_computer_move() {
        ...
        if(exports.is_clear(board)) {
            ...
            exports.minimax(board, true, true, 0);
            move = best_move;
        }
